...
from src.multi_turn_data_generation.attacker_models.AttackLM import AttackLM
...
class DefaultAttackLM(AttackLM):
    """
    ...
    def __init__(self, model: LanguageModel, max_n_attack_attempts: int, model_generation_params: dict,
                 n_streams: int, goal: str, target_str: str, keep_last_n: int, *kwargs: Any) -> None:
        super().__init__(model, max_n_attack_attempts, model_generation_params,
                         n_streams, goal, target_str, keep_last_n, *kwargs)

    def get_init_msg(self, goal, target):

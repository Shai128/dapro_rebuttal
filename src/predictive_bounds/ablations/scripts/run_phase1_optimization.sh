...
for ((job = 0; job < JOB_COUNT; job++)); do
  ...
  "${PYTHON_BIN}" -m src.predictive_bounds.ablations.phase1_optimization \
    "${COMMON_ARGS[@]}" \
...
if (( failed != 0 )); then
  echo "At least one Phase-I ablation worker failed; refusing to merge incomplete shards." >&2
  exit 1
fi

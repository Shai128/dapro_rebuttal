...
from src.predictive_bounds.bandit_algorithms.abstract_bandit_algorithm import BanditAlgorithm
from src.predictive_bounds.budget_allocators.budget_allocator import BudgetAllocator, BudgetAllocationResult
from src.predictive_bounds.calibration.calibration_utils import get_prior, solve_optimization, sample_calibration_set
...
class BanditsBudgetAllocator(BudgetAllocator):
    ...
    def allocate_budget(self, probability_est: torch.Tensor, x: torch.Tensor, t: torch.Tensor,
        ...
        # t and prior_quantile_est are both one-based interaction counts.
        # Quantiles are converted from grid indices at the common model-output
        # boundary, so adding one again here made this allocator run one turn
        # beyond its requested horizon.
        max_c = torch.minimum(t, torch.minimum(prior_quantile_est, torch.ones_like(prior_quantile_est) *

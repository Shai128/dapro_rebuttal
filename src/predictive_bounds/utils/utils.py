...
from src.dataset_utils.temporal import labels_from_event_times
...
from src.predictive_bounds.survival_utils.conditional_pmf_utils import get_conditional_pmf
from src.predictive_bounds.survival_utils.quantiles import compute_conditional_quantiles_single_step
from src.predictive_bounds.calibration.calibration_utils import quantiles_to_interaction_counts
...
def get_model(model_save_path, model_figure_save_path, is_real, x_train, t_tilde_train, device, features_size: int,
    ...
    dropout = 0.1
    # Model checkpoints are loaded through a PartialSequenceDataset, which
    # still needs labels for its observation bookkeeping.  t_tilde is a
    # one-based count; T+1 means no event.  The previous implementation used
    # min(0, t_tilde), incorrectly placing every event in y[:, 0].
    y_train = labels_from_event_times(
        t_tilde_train.to(device), horizon=x_train.shape[1]
    )
...
def multi_time_probabilities_compute_and_save_metrics(preds, t_tilde_test, delta_test,
                                                      output_dir):
    # Survival utilities return zero-based grid coordinates; reported event
    # times are one-based interaction counts.
    estimated_mean_time = (
        compute_mean_survival_time(preds, tail_distribution="geometric") + 1
    )
    ...
    width = preds.shape[-1] - 1
    q_025, q_05, q_1, q_9, q_95, q_975 = (
        quantiles_to_interaction_counts(q, width=width)
        for q in (q_025, q_05, q_1, q_9, q_95, q_975)
    )
...
def mean_time_compute_metrics(estimated_mean_time, t_test, delta_test):
    ...
    for t in range(0, estimated_mean_time.shape[1], 5):
        at_risk = (t_test > t) & delta_test.bool()
        mse_at_time_t = ((estimated_mean_time[:, t] - t_test)[at_risk] ** 2).mean().item()
        mae_at_time_t = ((estimated_mean_time[:, t] - t_test)[at_risk]).abs().mean().item()
        bias_at_time_t = abs((estimated_mean_time[:, t][at_risk].mean().item() - \
                              t_test[at_risk].float().mean().item()))
...
def setup_experiment_data(
    ...
    model_cal_test_preds_path = (
        ...
        else os.path.join(model_preds_dir, "probability_est_cal_test.pt")
    )

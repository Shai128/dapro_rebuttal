"""Construct LPBs calibrated on one AutoIF class and tested on another."""
...
def run_autoif_cross_class_experiments(
    ...
    for seed in tqdm.tqdm(
        ...
        if paper_methods_only:
            requested = set(paper_calibration_names(bound_type))
            available = {method.name for method in all_calibrations}
            missing = sorted(requested - available)
            if missing:
                raise ValueError(
                    f"Paper methods are unavailable: {missing}."
                )
            all_calibrations = [
                method for method in all_calibrations
                if method.name in requested
            ]
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        default=False,
        help="Run only the definitive paper comparison methods, including the oracle.",
    )
    parser.add_argument(
        "--no-skip-existing",
        dest="skip_existing",

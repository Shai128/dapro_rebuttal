...
param(
    ...
    [string]$SeedEnd = $(if ($env:SEED_END) { $env:SEED_END } else { "50" }),
    [string]$Suffix = $(if ($env:EXPERIMENT_SUFFIX) { $env:EXPERIMENT_SUFFIX } else { "definitive_dapro_v1" })
)

$ErrorActionPreference = "Stop"

$FinalName = "calibration_dapro_variance_aligned_bins_2_alpha_0p10_global_0p001_projection_margin_1p00_n1_200_allocation"
$RandomName = "calibration_random_adaptive_optimized_allocation"
$Methods = "$FinalName,$RandomName"

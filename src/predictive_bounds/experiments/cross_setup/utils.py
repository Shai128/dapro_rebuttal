"""Load and validate source-model and evaluation-setup experiment inputs."""
...
def setup_source_model_evaluation_data(
        cal_size: int,
        is_real: bool,
        device,
    ...
    """Apply a source survival model to a possibly different dataset/setup."""
    from src.dataset_utils.data_utils import get_data
    from src.predictive_bounds.utils.utils import compute_probabilities_and_quantiles

    cache_key = _short_hash(
        f"{is_real}\0{model_dataset_name}\0{evaluation_dataset_name}"
        f"\0{model_dataset_setup}\0{evaluation_dataset_setup}"
    )
    ...
    metadata_path.write_text(
        json.dumps(
            {
                "experiment_type": "cross_setup_prediction_cache",
    ...
    source_model_dir = (
        ...
        / f"is_real_{is_real}_dataset_{model_dataset_name}_dataset_{model_dataset_setup}"
    )

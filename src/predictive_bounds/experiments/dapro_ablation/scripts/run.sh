...
BUDGET_N1_VALUES=(25 50 100 150 200 250)
...
mapfile -t MERGED < <(find results/merged_calibration_dfs -type f \
  \( -path "*_${BASE_EXPERIMENT_SUFFIX}_n1/all_df.csv" \
     -o -path "*_${BASE_EXPERIMENT_SUFFIX}_score_noise/all_df.csv" \
     -o -path "*_${BASE_EXPERIMENT_SUFFIX}_budget/all_df.csv" \) | sort)
if (( ${#MERGED[@]} > 0 )); then

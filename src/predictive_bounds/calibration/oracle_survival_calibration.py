...
class OracleSurvivalCalibration(SurvivalLPBCalibration):
    ...
    def calibrate(self, x_cal: torch.Tensor, t_cal: torch.Tensor, model_prediction_cal: ModelPrediction):
        ...
        full_observation_cost = t_cal.reshape(-1).to(torch.float64).clamp(
            max=horizon
        )
        allocation_result = BudgetAllocationResult(
            ...
            C=C,
            C_probs=C_probs,
            total_budget_used=full_observation_cost.sum().item(),
            mean_weight=1.0,
            max_weight=1.0,
            additional_metrics={
                "infinite_budget_oracle": 1,
                "total_expected_budget": full_observation_cost.sum().item(),
                "total_expected_budget_per_sample": (
                    full_observation_cost.mean().item()
                ),
                "expected_budget_cost_semantics": "observe_until_event_or_horizon",
            },
        )
    ...
    def compute_metrics(self, model_prediction, target_taus) -> Dict[str, float]:
        ...
        inverse_probability = torch.ones(
            len(t), dtype=torch.float64, device=t.device
        )
...
class OracleSurvivalUPBCalibration(SurvivalUPBCalibration):
    """Exact UPB calibration with every calibration trajectory observed.

    This is an oracle benchmark, not a feasible allocation policy.  It uses
    unit inverse-probability weights and records the actual number of turns
    required to reveal every calibration outcome.
    """
    ...
    def calibrate(
        ...
        if not isinstance(model_prediction_cal, SurvivalModelPrediction):
            raise TypeError("Oracle UPB calibration needs survival predictions.")
        f = model_prediction_cal.quantile_est
        C = torch.full(
            (len(f),), float("inf"), dtype=f.dtype, device=f.device
        )
        C_probs = torch.ones(len(f), dtype=f.dtype, device=f.device)
        horizon = int(model_prediction_cal.probability_est.shape[1])
        full_observation_cost = t_cal.reshape(-1).to(torch.float64).clamp(
            max=horizon
        )
        self.allocation_result = BudgetAllocationResult(
            f=f,
            C=C,
            C_probs=C_probs,
            total_budget_used=full_observation_cost.sum().item(),
    ...
    def get_calibrated_upb(
        ...
        return model_prediction.quantile_est[:, positions].squeeze()

    @property
    def name(self) -> str:
        return "oracle_survival_upb_calibration"
